...
name = st.text_input("Name : ", key = 1, placeholder="-- First name and last name--")

email = st.text_input("email ID : ", key = 2)

Password = st.text_input("password : ", key = 3, placeholder = "min 8 characters with atleast 1 uppercase", type = "password")

Age = st.number_input("Age : ", key = 4, min_value = 10, placeholder = "only for 10 yrs or above")

Profession = st.text_input("Profession : ", key = 5, placeholder= "student if not working")

Income = st.number_input("Income (per annum) : ", key = 6, min_value=0)
